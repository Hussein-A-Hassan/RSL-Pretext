...
class VideoResNet(nn.Module):
    ...
    def __init__(
        ...
        ### self.fc = nn.Linear(512 * block.expansion, num_classes)            # Original Code
        ...
        self.feature_pool = nn.MaxPool3d(kernel_size=(4, 4, 4), stride=(2, 2, 2))       # 9216 layer 3  
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        #===================================================================== # 
        if self.return_conv:
            x_features = self.feature_pool(x_features)
            ##x_features = self.avgpool(x_features)
            ##print(((x_features.view(x_features.shape[0], -1))).shape)
            return x_features.view(x_features.shape[0], -1)                    # 9216
